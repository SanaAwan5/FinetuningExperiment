...
class Client:
    ...
    def fit(self):
        H=self.model.fit(self.X,self.y,epochs=50)
        return H
...
for i in range(n_clients):
    clients.append(Client(names[i], X[i], y[i],model))
    #model.set_weights() = 0
...
newList = []
for x in weights:
    newList.append(x/n_clients)
model.set_weights(newList)
